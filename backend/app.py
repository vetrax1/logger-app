...
def log():
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("INSERT INTO logs (message) VALUES (%s);", (f"[{timestamp}] {msg}",))
    conn.commit()
    cur.close()
    conn.close()
...
def recent():
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, message, created_at FROM logs ORDER BY id DESC LIMIT 10;")
    rows = cur.fetchall()
    cur.close()
    conn.close()

    data = [{"id": r[0], "message": r[1], "created_at": str(r[2])} for r in rows]
    return jsonify(data)
